...
#define ICE_ROR(x, N) ((x >> N) | (x << (32 - N)))

// RFC 6234, 5.1
#define ICE_CH(x, y, z) ((x & y) ^ (~x & z))
#define ICE_MAJ(x, y, z) ((x & y) ^ (x & z) ^ (y & z))
#define ICE_BSIG0(x) (ICE_ROR(x, 2) ^ ICE_ROR(x, 13) ^ ICE_ROR(x, 22))
#define ICE_BSIG1(x) (ICE_ROR(x, 6) ^ ICE_ROR(x, 11) ^ ICE_ROR(x, 25))
#define ICE_SSIG0(x) (ICE_ROR(x, 7) ^ ICE_ROR(x, 18) ^ (x >> 3))
#define ICE_SSIG1(x) (ICE_ROR(x, 17) ^ ICE_ROR(x, 19) ^ (x >> 10))
...
public:
  ...
  std::string str()
  {
    ...
    for (std::size_t i = 0; i < 32; i++) {
      ...
      str[i * 2 + 0] = to_ascii(c >> 4);
      str[i * 2 + 1] = to_ascii(c & 0xF);
    }
    return str;
  }
...
private:
  ...
  void process() noexcept
  {
    ...
    for (std::size_t t = 16; t != 64; ++t) {
      W[t] = ICE_SSIG1(W[t - 2]) + W[t - 7] + ICE_SSIG0(W[t - 15]) + W[t - 16];
    }
    ...
    for (std::size_t t = 0; t != 64; ++t) {
      const std::uint32_t T1 = h + ICE_BSIG1(e) + ICE_CH(e, f, g) + K[t] + W[t];
      const std::uint32_t T2 = ICE_BSIG0(a) + ICE_MAJ(a, b, c);
      h = g;
      g = f;
      f = e;
      e = d + T1;
